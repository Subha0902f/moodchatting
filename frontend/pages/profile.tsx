...
const MODES: Mode[] = [
  { id: "chat",  name: "Chat",        icon: "💬", description: "Reach out and connect with friends",    action: "chat",  active: true },
  { id: "hype",  name: "Hype",        icon: "🔥", description: "Get motivated with daily affirmations", action: "hype"               },
  { id: "calm",  name: "Break Timer", icon: "🍵", description: "Take a moment to breathe and reset",    action: "calm"               },
  { id: "notes", name: "Notes",       icon: "📝", description: "Write down your thoughts",              action: "notes"              },
];
...
const ProfilePictureModal = ({ isOpen, onClose, onUpload, onRemove, currentUrl, userId }: ProfilePictureModalProps) => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!valid) { setError(validationError || "Invalid file"); setSelectedFile(null); return; }
  ...
  const handleSave = async () => {
    if (!selectedFile) { setError("Select a file before saving."); return; }
    if (!userId) { setError("Unable to determine current user."); return; }
    setSaving(true);
    setError("");
    try {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "flex-end", zIndex: 50 }}>
      <div style={{ width: "100%", background: V.surface, borderTop: `1px solid ${V.border}`, borderRadius: "12px 12px 0 0", padding: "1.5rem 1.25rem", maxHeight: "80vh", overflow: "auto" }}>
        ...
        <button onClick={() => fileInputRef.current?.click()} disabled={saving} style={{ width: "100%", padding: "12px 0", borderRadius: 8, border: `1px solid ${V.lime}`, background: "transparent", color: V.lime, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
        ...
        {selectedFile && (
          <button onClick={handleSave} disabled={saving} style={{ width: "100%", marginTop: "0.75rem", padding: "12px 0", borderRadius: 8, border: "none", background: V.lime, color: V.bg, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
            {saving ? "Saving..." : "Save Photo"}
          </button>
        )}
        {currentUrl && !selectedFile && (
          <button onClick={handleRemove} disabled={saving} style={{ width: "100%", marginTop: "0.75rem", padding: "10px 0", borderRadius: 8, border: `1px solid ${V.error}`, background: "transparent", color: V.error, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.6 : 1 }}>
            {saving ? "Removing..." : "Remove Picture"}
          </button>
        )}
        <button onClick={onClose} style={{ width: "100%", marginTop: "0.75rem", padding: "10px 0", borderRadius: 8, border: "none", background: V.surface2, color: V.text, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Cancel
        </button>
      </div>
    </div>
...
const BioEditor = ({ isOpen, onClose, onSave, initialValue }: BioEditorProps) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "flex-end", zIndex: 50 }}>
      <div style={{ width: "100%", background: V.surface, borderTop: `1px solid ${V.border}`, borderRadius: "12px 12px 0 0", padding: "1.5rem 1.25rem", maxHeight: "80vh", overflow: "auto" }}>
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 14, fontWeight: 700, marginBottom: "1rem", color: V.text }}>Edit Bio</div>
        <textarea
          value={value}
          onChange={(e) => setValue(e.target.value)}
          maxLength={300}
          style={{ width: "100%", minHeight: 120, padding: 12, borderRadius: 8, border: `1px solid ${V.border}`, background: V.surface2, color: V.text, fontFamily: "'DM Sans', sans-serif", fontSize: 13, marginBottom: "1rem", resize: "vertical", boxSizing: "border-box" }}
          placeholder="Tell us about yourself..."
        />
        <div style={{ fontSize: 11, color: V.muted, marginBottom: "1rem" }}>{value.length}/300</div>
        <button onClick={handleSave} disabled={saving} style={{ width: "100%", padding: "12px 0", borderRadius: 8, border: "none", background: V.lime, color: V.bg, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.5 : 1, marginBottom: "0.75rem" }}>
          {saving ? "Saving..." : "Save Bio"}
        </button>
        <button onClick={onClose} style={{ width: "100%", padding: "10px 0", borderRadius: 8, border: "none", background: V.surface2, color: V.text, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Cancel
        </button>
      </div>
    </div>
...
interface HypeModalProps { isOpen: boolean; onClose: () => void; }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "flex-end", zIndex: 50 }}>
      <div style={{ width: "100%", background: V.surface, borderTop: `2px solid ${V.lime}`, borderRadius: "12px 12px 0 0", padding: "2rem 1.25rem", textAlign: "center" }}>
        ...
        <button onClick={onClose} style={{ width: "100%", padding: "12px 0", borderRadius: 8, border: "none", background: V.lime, color: V.bg, fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Awesome!
        </button>
      </div>
    </div>
...
interface CalmModalProps { isOpen: boolean; onClose: () => void; }
      ...
      setTimeLeft((prev) => {
        if (prev === null || prev <= 0) {
          ...
          if (Notification.permission === "granted") {
            new Notification("Break Timer", { body: "Your break is complete! 🍵" });
          }
          return null;
        }
        return prev - 1;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "flex-end", zIndex: 50 }}>
      <div style={{ width: "100%", background: V.surface, borderTop: `2px solid ${V.lime}`, borderRadius: "12px 12px 0 0", padding: "2rem 1.25rem", textAlign: "center" }}>
        ...
        <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 13, fontWeight: 700, color: V.lime, marginBottom: "1rem", textTransform: "uppercase", letterSpacing: "0.05em" }}>Break Timer</div>
        {!isRunning || timeLeft === null ? (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: "1.5rem" }}>
              {[5, 10, 15, 30, 45, 60].map((min) => (
                <button key={min} onClick={() => { setDuration(min); setTimeLeft(min * 60); setIsRunning(true); }}
            ...
            <button onClick={() => { setTimeLeft(duration * 60); setIsRunning(true); }}
          ...
          <>
            ...
            <button onClick={() => { setIsRunning(false); onClose(); }}
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (!profileUserId) { setRemoteProfile(null); setFriends([]); return; }
  ...
  const p = useMemo<ProfileData>(() => {
    ...
    return {
      ...
      bio: remoteProfile?.bio || metadata.bio || "Hi, I am a MOODCHAT user",
  ...
  const handleProfilePictureUpload = async (url: string) => {
    ...
    } catch (error) { console.error("Failed to save profile picture:", error); }
  };
  ...
  const handleProfilePictureRemove = async () => {
    ...
    } catch (error) { console.error("Failed to remove profile picture:", error); }
  };

  const handleBioSave = async (bio: string) => {
    try {
      await UserAPI.update({ bio });
      setRemoteProfile((prev) => ({ ...prev, bio }));
    } catch (error) { console.error("Failed to save bio:", error); }
  };
  ...
  const handleModeAction = (action: string) => {
    switch (action) {
      ...
      case "hype":  setHypeModalOpen(true); break;
      case "calm":  setCalmModalOpen(true); break;
      case "notes": navigate("/notepad"); break;
    }
  };

  const handleFriendClick = (friendId: string) => navigate(`/profile/${friendId}`);
    ...
    <div style={{ background: V.bg, fontFamily: "'DM Sans', sans-serif", color: V.text, minHeight: "100vh", paddingBottom: "3rem", maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{ background: `linear-gradient(180deg, ${V.surface} 0%, ${V.bg} 100%)`, padding: "2rem 1.25rem 0", position: "relative" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: "1rem", marginBottom: "1.25rem" }}>
          <div style={{ position: "relative", flexShrink: 0 }}>
            <div style={{ width: 84, height: 84, borderRadius: "50%", border: `2.5px solid ${V.lime}`, padding: 3, background: V.bg, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
              {p.profilePictureUrl
                ? <img src={p.profilePictureUrl} alt="profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 22, fontWeight: 700, color: V.lime }}>{p.initials}</div>
              }
            </div>
            {isOwnProfile && (
          ...
          <div style={{ flex: 1, paddingBottom: 4 }}>
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 19, fontWeight: 700, color: V.text, letterSpacing: "-0.01em", lineHeight: 1.1 }}>{p.fullName}</div>
            <div style={{ fontSize: 12, color: V.lime, marginTop: 3, fontFamily: "'Space Mono', monospace", letterSpacing: "0.04em" }}>{p.handle}</div>
            <div style={{ display: "flex", gap: "1.25rem", marginTop: 8 }}>
              {p.stats.map((s) => (
                <div key={s.label} style={{ textAlign: "center" }}>
                  <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 13, fontWeight: 700, color: V.lime }}>{s.value}</div>
                  <div style={{ fontSize: 10, color: V.muted, textTransform: "uppercase", letterSpacing: "0.06em" }}>{s.label}</div>
                </div>
              ))}
            </div>
            {isOwnProfile && (
              <button onClick={() => setUploadModalOpen(true)} style={{ marginTop: 14, borderRadius: 999, border: `1px solid ${V.border}`, background: V.surface2, color: V.text, fontSize: 12, fontWeight: 700, padding: "10px 14px", cursor: "pointer", letterSpacing: "0.03em" }}>
                Change photo
              </button>
            )}
          </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: "1.25rem" }}>
          {p.hashtags.map((tag) => (
            <span key={tag} style={{ fontSize: 11, fontFamily: "'Space Mono', monospace", padding: "4px 10px", borderRadius: 100, border: `1px solid ${V.border}`, color: V.lime, background: V.limeGlow, letterSpacing: "0.03em", cursor: "pointer" }}>{tag}</span>
          ))}
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: "1.5rem" }}>
          {[{ icon: <IconMail />, text: p.email }, { icon: <IconPhone />, text: p.phone }].map(({ icon, text }) => (
            <div key={text} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11.5, background: V.surface2, border: `1px solid ${V.surface3}`, borderRadius: 8, padding: "6px 10px", color: V.muted, flex: 1, overflow: "hidden" }}>
              {icon}
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 11 }}>{text}</span>
            </div>
          ))}
        </div>
        ...
        {isOwnProfile && (
          <div style={{ background: V.surface, border: `1px solid ${V.border}`, borderRadius: 12, padding: "1rem", marginBottom: "1.5rem" }}>
            <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: V.muted, marginBottom: 12, fontFamily: "'Space Mono', monospace" }}>Share options</div>
            <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                <div style={{ fontSize: 10, color: V.muted, padding: "0 2px" }}>{p.profileLink}</div>
              </div>
      ...
      <div style={{ padding: "1rem 1.25rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: V.text, marginBottom: 6 }}>Bio</div>
            <div style={{ fontSize: 13, color: V.muted, lineHeight: 1.6, maxWidth: "85%" }}>{p.bio || "No bio yet"}</div>
          </div>
          {isOwnProfile && (
            <button onClick={() => setBioModalOpen(true)} style={{ fontSize: 12, color: V.lime, background: "transparent", border: "none", cursor: "pointer", fontFamily: "'Space Mono', monospace", fontWeight: 700 }}>
              Edit
            </button>
          )}
        </div>
      ...
      {friends.length > 0 && (
        <>
          ...
          <div style={{ display: "flex", gap: 10, overflowX: "auto", padding: "0 1.25rem 0.75rem", scrollbarWidth: "none" }}>
            {friends.map((f) => (
              <div key={f.id} style={{ flexShrink: 0, width: 68, display: "flex", flexDirection: "column", alignItems: "center", gap: 5, cursor: "pointer" }} onClick={() => handleFriendClick(f.id)}>
                <div style={{ position: "relative" }}>
                  <div style={{ width: 52, height: 52, borderRadius: "50%", background: f.active ? V.limeGlow : V.surface3, border: `1.5px solid ${f.active ? V.lime : V.surface3}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Mono', monospace", fontSize: 13, fontWeight: 700, color: f.active ? V.lime : V.muted }}>
                    {f.initials}
                  </div>
                  {f.online && <div style={{ width: 8, height: 8, background: V.lime, borderRadius: "50%", border: `1.5px solid ${V.bg}`, position: "absolute", bottom: 1, right: 1 }} />}
                </div>
                <div style={{ fontSize: 10, color: V.muted, textAlign: "center", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", width: "100%" }}>{f.name}</div>
              </div>
            ))}
          </div>
          <SectionDivider />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "0 1.25rem" }}>
        {p.modes.map((m) => (
          <div key={m.id} onClick={() => handleModeAction(m.action)} style={{ background: m.active ? V.limeGlow : V.surface, border: `1px solid ${m.active ? V.lime : V.surface3}`, borderRadius: 10, padding: 12, cursor: "pointer" }}>
            <span style={{ fontSize: 20, display: "block", marginBottom: 6 }}>{m.icon}</span>
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, fontWeight: 700, color: m.active ? V.lime : V.text, marginBottom: 3 }}>{m.name}</div>
            <div style={{ fontSize: 10, color: V.muted, lineHeight: 1.4 }}>{m.description}</div>
            {m.active && <span style={{ display: "inline-block", fontSize: 9, fontFamily: "'Space Mono', monospace", padding: "2px 6px", borderRadius: 4, marginTop: 5, background: V.limeGlow, color: V.lime, letterSpacing: "0.04em" }}>ACTIVE</span>}
          </div>
        ))}
      </div>

      {/* Modals */}
      <ProfilePictureModal isOpen={uploadModalOpen} onClose={() => setUploadModalOpen(false)} onUpload={handleProfilePictureUpload} onRemove={handleProfilePictureRemove} currentUrl={p.profilePictureUrl} userId={user?.id} />
      <BioEditor isOpen={bioModalOpen} onClose={() => setBioModalOpen(false)} onSave={handleBioSave} initialValue={p.bio} />
